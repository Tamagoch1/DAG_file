...
args = {
    'owner': 'airflow',
    'start_date': dt.datetime(2025, 2, 11),
    'retries': 1,
    'retry_delay': dt.datetime(minute=1),
    'depends_on_past': False,
}

FILINAME = os.path.join(os.path.expanduser('~'), 'titanic.csv')

def download_titanic_dataset():
    url = 'https://web.stanford.edu/class/archive/cs/cs109/cs109.1166/stuff/titanic.csv'
    response = requests.get(url, stream=True)
    response.raise_for_status()
    with open(FILINAME, 'w', encoding='utf-8') as f:
        for chunk in response.iter_lines():
            f.write('{}\\n'.format(chunk.decode('utf-8')))

def pivot_dataset():
    titatinc_df = pd.read_csv(FILENAME)
    pvt = titatinc_df.pivot_table(
